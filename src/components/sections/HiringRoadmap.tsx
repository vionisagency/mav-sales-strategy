...
export default function HiringRoadmap({ calc, kpis }: Props) {
  // Ramp-up factor for Month 1–2: BDR is new, assume ~60% of full throughput
  const rampTouchesPerDay  = Math.max(1, Math.round((calc.totalTouches / 22) * 0.6))
  const rampConnectsPerDay = Math.max(1, Math.round((calc.totalConnections / 22) * 0.6))
  const rampDemosPerWeek   = Math.max(1, Math.round(calc.demosPerMonth / 4.33 * 0.5))
  ...
  const milestones = [
    {
      ...
      kpi: `${rampTouchesPerDay}–${rampTouchesPerDay + 20} touches/day · ${rampConnectsPerDay}–${rampConnectsPerDay + 3} connections/day · ${rampDemosPerWeek}+ demos booked/week`,
    ...
    },
    {
      ...
      kpi: `${calc.demosPerMonth}+ demos/month · ${kpis.warmConnectToDemo}%+ warm connect→demo rate`,
      ...
      tasks: [
        ...
        `Book ${calc.demosPerMonth}+ qualified demos/month for Sameet`,
    ...
    },
    {
      ...
      kpi: `${calc.demosPerMonth * 2}+ demos/month (2 AEs) · ${calc.bdrsNeeded} BDR${calc.bdrsNeeded !== 1 ? 's' : ''} needed to fill pipeline`,
      ...
      tasks: [
        ...
        `${calc.demosPerMonth * 2}+ demos/month total from combined AE capacity`,
      ],
    },
    {
      months: 'Month 6+',
      ...
      kpi: `${calc.demosPerMonth * 2}+ demos/month · ${calc.totalTouches.toLocaleString()} touches/month · tracked cold/warm/hot rates in Zoho`,
      zoho: 'Sales analytics dashboard, leaderboard, forecasting in Zoho',
      description:
        `2 AEs + ${calc.bdrsNeeded}–${calc.bdrsNeeded + 1} BDRs. Fully documented sales playbook. Sameet transitions to growth strategy, partnerships, and enterprise deals. Sales org runs without founder involvement in day-to-day.`,
      tasks: [
        '2 AEs running demos independently',
        `${calc.bdrsNeeded}–${calc.bdrsNeeded + 1} BDRs generating ${calc.demosPerMonth * 2}+ demos/month pipeline`,
    ...
    <section id="roadmap" className="min-h-screen py-24 px-12 bg-navy-900 flex flex-col justify-center">
      ...
      >
        <div className="flex items-center gap-3 mb-3">
          <span className="text-accent text-sm font-semibold tracking-wider uppercase">Section 03</span>
          <div className="h-px flex-1 bg-navy-600" />
        </div>
        <h2 className="text-4xl font-extrabold text-white mb-2">Hiring Roadmap</h2>
        <p className="text-slate-400 mb-3 text-lg">
          From 1 founder-seller to a full sales team in 6 months.
        </p>
        <p className="text-slate-500 text-sm mb-12 flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-accent inline-block" />
          KPIs update live from the Sales Simulator — adjust the inputs to see this roadmap change.
        </p>
                ...
                <div className="flex flex-col gap-4">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <div className="flex items-center gap-3 mb-1">
                        ...
                        <h3 className="text-white font-bold text-lg">{m.role}</h3>
                      </div>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <div className="bg-navy-800/50 rounded-xl p-3">
                      <p className="text-[10px] text-slate-500 uppercase tracking-wider mb-1 font-bold">KPI Target</p>
                      <p className="text-xs text-white font-medium leading-relaxed">{m.kpi}</p>
                    </div>
                    {/* Zoho unlock */}
                    <div className="bg-navy-800/50 rounded-xl p-3">
                      <p className="text-[10px] text-slate-500 uppercase tracking-wider mb-1 font-bold">Zoho CRM Setup</p>
                      <p className="text-xs text-white font-medium leading-relaxed">{m.zoho}</p>
                    </div>
                    {/* Tasks */}
                    <div className="bg-navy-800/50 rounded-xl p-3">
                      <p className="text-[10px] text-slate-500 uppercase tracking-wider mb-1 font-bold">Key Actions</p>
